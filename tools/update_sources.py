"""
Module: update_sources.py
Description:
    Updates the 'Source' select in Notion (e.g., Manga, Light Novel, Original) using AniList metadata.

Usage:
    python cli.py tools update-sources [--titles file.txt]
...
# Mapping AniList source → Notion select value
SOURCE_MAP = {
    "MANGA": "Manga",
...
def request_with_retry(url, headers, json_data=None, retries=3, delay=2):
    """Perform a POST request with retries in case of errors."""
...
def get_notion_anime():
    """Fetch anime entries from Notion that have an empty 'Source' property."""
    all_anime = []
    start_cursor = None
    ...
    while True:
        ...
        for page in results.get("results", []):
            ...
            source = source_data.get("name", "") if source_data else ""

            if source:  # Skip already updated entries
                continue
...
def update_source_property(page_id, source):
    """Update the Notion 'Source' property for a given page."""
    try:
        notion.pages.update(page_id=page_id, properties={"Source": {"select": {"name": source}}})
        console.print(f"[bold green]✅ Source updated: {source}[/bold green]")
    except Exception as e:
        console.print(f"[bold red]❌ Failed to update Source: {e}[/bold red]")


def search_anilist(title, debut_year):
    """Search AniList for a given title and debut year."""
    variables = {
        "search": title,
...
def main(titles_file: str | None = None):
    ...
    for idx, anime in enumerate(anime_list, start=1):
        ...
        if len(perfect_matches) == 1:
            ...
            source_raw = matched.get("source")
            if source_raw is None:
                choice = Prompt.ask(
                    "[bold yellow]Source is missing on AniList. Set to 'N/A'?[/bold yellow]",
                    choices=["y", "n"],
                )
                if choice == "y":
                    update_source_property(anime["id"], "N/A")
                    updated += 1
                    accepted_na.append(anime)
                else:
                    not_matched.append(anime)
                continue

            source_clean = SOURCE_MAP.get(source_raw, "Other")
            console.print(
                f"[bold green]Auto-matched:[/bold green] {title} → [italic]{source_clean}[/italic]"
            )
            update_source_property(anime["id"], source_clean)
            updated += 1
            continue
        ...
        source_raw = selected_match.get("source", "OTHER")
        source_clean = SOURCE_MAP.get(source_raw, "Other")
        update_source_property(anime["id"], source_clean)
        updated += 1

    if not_matched:
        csv_path = output_dir / "unmatched_sources.csv"
    ...
    if accepted_na:
        csv_path = output_dir / "accepted_na_sources.csv"
        with open(csv_path, "w", newline="", encoding="utf-8") as csvfile:
            writer = csv.DictWriter(csvfile, fieldnames=["eng_title", "format", "debut_year"])
            writer.writeheader()
            for anime in accepted_na:
                writer.writerow(
                    {
                        "eng_title": anime["eng_title"],
                        "format": anime["format"],
        ...
        console.print(f"\n[bold cyan]📄 Titles updated with 'N/A' saved to:[/bold cyan] {csv_path}")
