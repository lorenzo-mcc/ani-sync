"""
Module: notion_updater.py
Description:
    Notion update helpers for creating and updating pages and properties.
...
def create_notion_page(database_id, properties, genre_database_id, force=False):
    try:
        ...
        if properties.get("Format") not in format_excluded:
            notion_properties["Seasons Watched"] = {"number": properties.get("Seasons Watched", 0)}
            notion_properties["Episodes Watched"] = {
                "number": properties.get("Episodes Watched", 0)
            }
